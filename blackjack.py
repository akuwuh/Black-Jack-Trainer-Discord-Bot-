...
class Game:
    ...
    def button_view(self):
        ...
        double_stand = Button(label="Double/Stand", style=nextcord.ButtonStyle.primary)
        split = Button(label="Split", style=nextcord.ButtonStyle.primary)
        split_das = Button(label="Split (DAS only)", style=nextcord.ButtonStyle.primary)
        ...
        async def update_embed(message, interaction):
            ...
            new_embed = embed = nextcord.Embed(
            colour=nextcord.Colour.purple()
            )  
            new_embed.set_thumbnail(url="https://png.pngtree.com/png-vector/20220812/ourmid/pngtree-blackjack-png-image_6107450.png")
            
            new_embed.description =  message

            play_again_button = nextcord.ui.Button(style=nextcord.ButtonStyle.primary, label="Play Again", custom_id="play_again")
    ...
    def eval(self,action): # this is so hard 
        ...
        if self.is_pair and not (pairs[self.card1_val][self.dealer_val - 2] == "N" and action != "Y" and action != "Yn"): # keep evaluating
                return (pairs[self.card1_val][self.dealer_val - 2] == action, answer_conv[pairs[self.card1_val][self.dealer_val - 2]])

        
        if max(self.card1_val, self.card2_val) == 11: # soft total
            return eval_soft(min(self.card1_val, self.card2_val), self.dealer_val - 2)
        else: # hard total 
    ...
    def generate_card(self,dealer_card=False):
        if self.card1 != None and self.card1[0] == "A" and dealer_card == False: # if first card is ace, dont give jqk
            random_card = random.choice(list(cards.keys())[1:8]) # removed ace (first element)
        elif (self.card1 != None and (self.card1[0] == "J" or self.card1[0] == "Q" or self.card1[0] == "K")) and dealer_card == False: # if first card is jqk, dont give ace
            random_card = random.choice(list(cards.keys())[1:8]) # removed last 3 element(jqk)
        else:
